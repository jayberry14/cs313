...
#define ASSGHOME "/mnt/local/testbed/"
...
#define GRADESUFFIX ".grade"

   // --- The suffix that the compile program specified in the assignment file
   //     appends to each student's compiler output file.
#define COMPILESUFFIX ".compile"
...
#define BOLD "\E[0;1m" //[1m"
#define UL "\E[0;4m" //[4m"
#define REVERSE "\E[0;7m" //[7m"
#define BEEP ""
#define RESET "\E[0m" //[m"
...
*
* Note:
*    Everything is declared static in this class because the alarm() function
...
*******************************************************************************/
{
   ...
   public:
      ...
      static bool testExit(bool exitIsEarly, int &penalty,
...
/******************************************************************************
* Summary:
*    Executes a student's program in its own process.  Pipes are established
*    to that process.
* Inputs:
...
*******************************************************************************/
{
   ...
   outStream = fdopen(outPipe[1], "w");
      // --- Eliminate buffers on inStream and outStream.  (I've never verified
...
*******************************************************************************/
{
   ...
   if (retVal == -1)
   {
      cerr << "Error killing child process" << endl;
   }

      // --- If the signal is a keyboard interrupt
   if (signalNum == SIGINT)
   {
      cerr << MYNAME << ": terminated by keyboard interrupt" << endl;
      exit(0);
   }
...
bool Process::testExit(bool exitIsEarly, int &penalty, char *comment,
                       int testNum)
/******************************************************************************
* Summary:
...
* Inputs:
*    exitIsEarly - True if a penalty is to be assigned
...
*******************************************************************************/
{
   ...
   {
      cout << "These are HELP instructions" << endl;
      exit(1);
   }
   ...
   if (argc >= argCount)
   {
      ...
      strcpy(myAssg.assgDir, ASSGHOME);
      ...
      {
         cerr << MYNAME << ": Invalid assignment name" << endl;
      }
   }
   else  // --- If there were no more items to be read from command line
   {
      cerr << MYNAME << ": Invalid command line " << endl;
      exit(1);
   }
   ...
   {
      cerr << MYNAME << ": Invalid source file list" << endl;
      exit(1);
   }
...
*******************************************************************************/
{
   ...
   if (assgFile.bad())
   {
      cerr << MYNAME << ": Assignment definition file could not be opened"
           << endl;
...
//   while (! assgFile.eof())
   ...
   while (assgFile.peek() != EOF)
   {
    char *tempPtr;
      ...
      if (! tempPtr)
      {
         cerr << MYNAME << ": In assg file - tag expected on line " << lineCount 
              << endl;
         exit(1);
      }
      ...
      if (! tempPtr)
      {
         cerr << MYNAME << ": In assg file - Semicolon missing on line " 
              << lineCount << endl;
         exit(1);
      }
      ...
      else if (! strcmp(inLine, "ResumeFromCrash"))
      {
         ...
         cerr << MYNAME << ": Unknown tag in assignment file on line " 
              << lineCount << endl;
...
*******************************************************************************/
{
   ...
   if(!testFile)
   {
      cerr << "Error: Test instructions file could not be found/opened." << endl
           << endl;
      exit(1);
   }
   ...
   {
      cerr << MYNAME << ": Test instructions file could not be opened" << endl;
      cerr << "\tPlease specify one (e.g., \"" << MYNAME 
           << " cs235/hw01 -myTest.test myCode.cpp\" or " << endl 
           << "\t\"" << MYNAME << " cs165/hw16 myCode.cpp\")" << endl;
      exit(1);
   }
...
*******************************************************************************/
{
   ...
   {
      cerr << MYNAME << ": First test instruction is not '+'" << endl;
      exit(1);
   }
   ...
   while (instructs[index] != NULL)
   {
      ...
      else if (instructs[index]->stop)
      {
         ...
         if (retChar)
            ...
            didExit = Process::testExit(false, penalties[penaltyNum].penalty ,
      ...
      else if (instructs[index]->recv)
      {
         ...
         else  // --- If no output was available
         {
            ...
            didExit = Process::testExit(true, penalties[penaltyNum].penalty ,
                                        penalties[penaltyNum].comment, testNum);
...
*******************************************************************************/
{
   ...
   if (myAssg.gradeMode)
   {
      ...
      logFile.open(CLASSLOGFILE, ios::out | ios::app);
   }
   ...
   cout << BOLD << UL << "\n\n" << myAssg.sourceFiles[myAssg.sourceIndex]
        << "  --  ";
   cout << timeLabel << RESET << endl;
   outFile << BOLD << UL << "\n\n" << myAssg.sourceFiles[myAssg.sourceIndex]
           << "  --  ";
   outFile << timeLabel << RESET << endl;
   logFile << BOLD << UL << "\n\n" << myAssg.sourceFiles[myAssg.sourceIndex]
           << "  --  ";
   logFile << timeLabel << RESET << endl;
   ...
   while (instructs[index] && results[rindex])
   {
      ...
      if (results[rindex]->restart)
      {
         ...
         cout << BOLD << endl << "<Program Restarted>" << RESET;
         outFile << BOLD << endl << "<Program Restarted>" << RESET;
         logFile << BOLD << endl << "<Program Restarted>" << RESET;
      ...
      {
         cout << endl << BOLD << "<Program Started>" << RESET;
         outFile << endl << BOLD << "<Program Started>" << RESET;
         logFile << endl << BOLD << "<Program Started>" << RESET;
      }
         ...
         {
            cout << endl << BOLD << "<Program Ended>" << RESET << endl;
            outFile << endl << BOLD << "<Program Ended>" << RESET << endl;
            logFile << endl << BOLD << "<Program Ended>" << RESET << endl;
         }
         ...
         if (results[rindex]->send)
         {
            ...
            if (strcmp(instructs[index]->send, results[rindex]->send) == 0)
            {
               ...
               cout << UL << results[rindex]->send << RESET;
               outFile << UL << results[rindex]->send << RESET;
               logFile << UL << results[rindex]->send << RESET;
            }
         }
         ...
         {
            cout << endl << REVERSE << "Could not send: "
                 << instructs[index]->send << RESET << endl;
            outFile << endl << REVERSE << "Could not send: "
                    << instructs[index]->send << RESET << endl;
            logFile << endl << REVERSE << "Could not send: "
                    << instructs[index]->send << RESET << endl;
         }
         ...
         if (results[rindex]->recv)
         {
            ...
            cout << endl << BOLD << setw(3) << testNum << "> " << RESET
                 << "|" << results[rindex]->recv << "|" << flush;
            outFile << endl << BOLD << testNum << setw(3) << "> " << RESET
                 << "|" << results[rindex]->recv << "|" << flush;
            logFile << endl << BOLD << testNum << setw(3) << "> " << RESET
                 << "|" << results[rindex]->recv << "|" << flush;
            ...
            if (!equal)
            {
               ...
               cout << endl << REVERSE << "Exp: "
                    << "|" << instructs[index]->recv << "|" << RESET;
               outFile << endl << REVERSE << "Exp: "
                    << "|" << instructs[index]->recv << "|" << RESET;
               logFile << endl << REVERSE << "Exp: "
                    << "|" << instructs[index]->recv << "|" << RESET;
            }
         }
   ...
   {
      cout << BOLD << endl << "<Program Terminated>" << RESET << endl;
      outFile << BOLD << endl << "<Program Terminated>" << RESET << endl;
      logFile << BOLD << endl << "<Program Terminated>" << RESET << endl;
   }

      // --- Output a list of all penalties accumulated during testing
   cout << '\n' << UL << "ERROR SUMMARY:" << RESET << '\n' << endl;
   outFile << '\n' << UL << "ERROR SUMMARY:" << RESET << '\n' << endl;
   logFile << '\n' << UL << "ERROR SUMMARY:" << RESET << '\n' << endl;
...
*******************************************************************************/
{
   ...
   Penalty *myPenalties = new Penalty[MAXPENALTIES];
   char fileName[100];
   int index;

      // --- Erase the class log file if it exists.  I can't remember why this
      //     was necessary.
   unlink(CLASSLOGFILE);
